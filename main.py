...
# Database setup - Render provides DATABASE_URL automatically
DATABASE_URL = os.getenv("DATABASE_URL")
if not DATABASE_URL:
    raise RuntimeError("DATABASE_URL environment variable is not set")

print(f"[DEBUG] Connecting to database: {DATABASE_URL[:30]}...")
...
def send_whatsapp_message(to_phone: str, message: str):
    ...
    try:
        ...
        print(f"[DEBUG] WhatsApp message sent successfully: {msg.sid}")
        return True
    ...
    <body>
        ...
        <div class="container">
            <div class="sidebar">
                ...
                <a href="/admin/organizations" class="{'active' if active_menu == 'organizations' else ''}">Organizations</a>
                <a href="/admin/invoices" class="{'active' if active_menu == 'invoices' else ''}">Invoices</a>
                <a href="/admin/purchases" class="{'active' if active_menu == 'purchases' else ''}">Customer Purchases</a>
                <a href="/admin/refurbs" class="{'active' if active_menu == 'refurbs' else ''}">Refurbs</a>
                <a href="/admin/tickets" class="{'active' if active_menu == 'tickets' else ''}">Tickets</a>
                <a href="/admin/parts" class="{'active' if active_menu == 'parts' else ''}">Parts</a>
                <a href="/admin/inventory" class="{'active' if active_menu == 'inventory' else ''}">Inventory</a>
                <a href="/admin/purchase-orders" class="{'active' if active_menu == 'purchase-orders' else ''}">Purchase Orders</a>
                <a href="/admin/pos" class="{'active' if active_menu == 'pos' else ''}">POS</a>
                <a href="/admin/reports" class="{'active' if active_menu == 'reports' else ''}">Reports</a>
                <a href="/admin/admin" class="{'active' if active_menu == 'admin' else ''}">Admin</a>
                <a href="/admin/help" class="{'active' if active_menu == 'help' else ''}">Help</a>
...
async def root():
    """Public homepage - bookings only in-store"""
    return """
    <!DOCTYPE html>
    <html>
    <head>
...
async def dashboard(request: Request, user = Depends(require_auth)):
    ...
    <div class="get-started">
        <a href="/admin/customers/new" class="action-btn">+ New Customer</a>
        <a href="/admin/tickets/new" class="action-btn">+ New Ticket</a>
        <a href="/admin/checkin" class="action-btn">+ New Check In</a>
        <a href="/admin/invoices/new" class="action-btn">+ New Invoice</a>
        <a href="/admin/estimates/new" class="action-btn">+ New Estimate</a>
    </div>
    ...
    <script>
        ...
        new Chart(paymentsCtx, {
            ...
            options: {
                ...
                plugins: { legend: { labels: { color: '#ccc' } } },
                scales: {
                    y: { beginAtZero: true, ticks: { color: '#ccc', callback: function(value) { return '£' + value; } }, grid: { color: '#333' } },
                    x: { ticks: { color: '#ccc' }, grid: { color: '#333' } }
                }
            }
        });
...
@app.get("/admin/invoices/new", response_class=HTMLResponse)
...
@app.get("/admin/refurbs", response_class=HTMLResponse)
...
@app.get("/admin/parts", response_class=HTMLResponse)
...
@app.get("/admin/pos", response_class=HTMLResponse)
...
@app.get("/admin/estimates/new", response_class=HTMLResponse)
...
async def customers_list(request: Request, user = Depends(require_auth), db: Session = Depends(get_db)):
    ...
    if customers:
        for customer in customers:
            ...
            check_in_btn = "" if customer.check_in_at else f'<form method="post" action="/admin/customers/{customer.id}/checkin" style="display:inline;"><button type="submit" style="background:#00C4B4;color:#000;border:none;padding:6px 12px;border-radius:4px;cursor:pointer;font-weight:bold;">Check-in</button></form>'
    ...
    <style>
        ...
        .btn-primary {{
            background: #00C4B4; color: #000; padding: 12px 24px; border: none;
            border-radius: 5px; font-weight: bold; text-decoration: none;
            display: inline-block; cursor: pointer; margin-left: 20px;
        }}
        ...
        th {{ background: #333; padding: 15px; text-align: left; color: #00C4B4; font-weight: bold; border-bottom: 2px solid #00C4B4; }}
    ...
    <table>
        <thead>
            <tr>
                <th>ID</th><th>Name</th><th>Business Name</th><th>Email</th>
                <th>Phone</th><th>Created At</th><th>Check-in At</th><th>Action</th>
            </tr>
        </thead>
...
async def new_customer_form(request: Request, user = Depends(require_auth)):
    content = """
    <style>
        .form-card { background: #222; padding: 30px; border-radius: 8px; border: 1px solid #333; max-width: 600px; }
        .form-group { margin-bottom: 20px; }
        label { display: block; color: #ccc; margin-bottom: 8px; font-size: 14px; }
        label .required { color: #ff6b6b; }
        input { width: 100%; padding: 12px; background: #333; border: 1px solid #444; border-radius: 5px; color: #fff; font-size: 14px; }
        input:focus { outline: none; border-color: #00C4B4; }
        .form-actions { display: flex; gap: 15px; margin-top: 30px; }
        button { padding: 12px 30px; border: none; border-radius: 5px; font-size: 16px; font-weight: bold; cursor: pointer; transition: all 0.2s; }
        .btn-submit { background: #00C4B4; color: #000; }
        .btn-submit:hover { background: #00d4c4; }
    ...
    <div class="form-card">
        <form method="post" action="/admin/customers/new">
            ...
            <div class="form-actions">
                ...
                <button type="button" class="btn-cancel" onclick="window.location.href='/admin/customers'">Cancel</button>
            </div>
        </form>
    </div>
    """
...
async def create_customer(
    ...
    try:
        ...
        # Redirect to new ticket form with customer ID
        return RedirectResponse(url=f"/admin/tickets/new?customer_id={customer.id}", status_code=302)
    except Exception as e:
        print(f"[ERROR] Failed to create customer: {e}")
        db.rollback()
        raise HTTPException(status_code=500, detail="Failed to create customer")

@app.post("/admin/customers/{customer_id}/checkin")
...
async def tickets_list(request: Request, user = Depends(require_auth), db: Session = Depends(get_db)):
    ...
    <div class="header-actions">
        ...
        <a href="/admin/tickets/new" class="btn-primary">+ New Ticket</a>
    </div>
    
    <table>
        <thead>
            <tr>
                <th>ID</th><th>Customer</th><th>Device Type</th>
                <th>Brand/Model</th><th>Fault Type</th><th>Status</th><th>Created</th>
            </tr>
        </thead>
        <tbody>{rows_html}</tbody>
    </table>
...
async def new_ticket_form(request: Request, customer_id: Optional[int] = None, user = Depends(require_auth), db: Session = Depends(get_db)):
    customer_name = ""
    customer_phone = ""
    customer_email = ""
    ...
    if customer_id:
        try:
            ...
            print(f"[ERROR] Failed to fetch customer: {e}")
    
    content = f"""
    <style>
        .form-card {{ background: #222; padding: 30px; border-radius: 8px; border: 1px solid #333; max-width: 800px; }}
        .form-group {{ margin-bottom: 20px; }}
        label {{ display: block; color: #ccc; margin-bottom: 8px; font-size: 14px; }}
        label .required {{ color: #ff6b6b; }}
        input, textarea, select {{ width: 100%; padding: 12px; background: #333; border: 1px solid #444; border-radius: 5px; color: #fff; font-size: 14px; }}
        textarea {{ min-height: 100px; resize: vertical; }}
        input:focus, textarea:focus, select:focus {{ outline: none; border-color: #00C4B4; }}
    ...
    <h1>New Ticket</h1>
    <div class="form-card">
        <form method="post" action="/admin/tickets/new">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="tel" name="customer_phone" value="{customer_phone}" required placeholder="+44...">
                </div>
            </div>
            ...
            <div class="form-group">
                <label>Fault Type <span class="required">*</span></label>
            ...
            <div class="form-group">
                <label>Additional Details <span class="required">*</span></label>
                <textarea name="faults" required placeholder="Describe the problem in detail..."></textarea>
            </div>
            ...
            <div class="form-actions">
                ...
                <button type="button" class="btn-cancel" onclick="window.location.href='/admin/tickets'">Cancel</button>
            </div>
        </form>
    </div>
    """
...
async def create_ticket(
    ...
    print(f"[DEBUG] Creating ticket for: {customer_name} - {device_type} - {fault_type}")
    ...
    try:
        ...
        message = f"Hi {customer_name}, your repair ticket #{ticket.id} has been created for {brand} {model} ({fault_type}).{cost_text} We'll update you soon! - Quick Click Repairs"
